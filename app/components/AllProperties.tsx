...
const AllProperties: React.FC<AllPropertiesProps> = ({
  ...
  const onDeleteImage = (imageUrl: string, id: string) => {
    ...
    axios
      .delete(`/api/listings/delete/${id}`, { data: { listingId: id, imageUrl } }) // Pass parameters in the request body
      .then(() => {
        ...
        <tbody>
          {listings.map((listing: SafeListing) => (
            <tr key={listing.id} className="border-b">
              ...
              <td className="py-2 px-4 justify-center items-center text-center font-semibold text-lg">
              ...
              <td className="py-2 px-4 justify-center items-center text-center font-bold">
                {editingId === listing.id ? (
                  <input
                    type="text"
                    value={editablePrice}
        ...
        <div className="z-50 fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white p-8 overflow-y-auto rounded-lg h-[600px]">
            ...
            <div className="flex justify-center items-center flex-col ">
              <div
                className="top-4 text-red-500 cursor-pointer"
                onClick={closeImagesModal}
              >
                X
              </div>
              {selectedImages.map((imageUrl, index) => (
                <div
                  key={index}
                  className="relative w-[400px] h-[400px] m-2 group"
                >
                  <Image
                    ...
                    objectFit="contain"
                  />
                  <div className="cursor-pointer absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out bg-black bg-opacity-50 text-white">
                    <div className="flex flex-col items-center">
                      ...
                      <button
                        ...
                        onClick={() => onDeleteImage(imageUrl, listings[index].id)}
                      >
                        Delete
                      </button>
                    </div>
